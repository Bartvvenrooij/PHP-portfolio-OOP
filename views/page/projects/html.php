...
?>
    ...
    <section class="comment">
        <?php
        if ($search == false) {
            if (count($comment->getCommentsByType('Html')) > 0) {
                foreach ($comment->getCommentsByType('Html') as $com) {
                    echo '<article>';
                    ...
                    if (Session::exists('user')) {
                        if ($user->getGroup() == 'Administrator') {
                            echo '<a href="?link=8&page=1&deleteComment=' . $com->idcomments . '">Delete comment</a>';
                        }
                    }
                    echo '</p></article>';
                }
            ...
            $results = $comment->getCommentsByMessage('Html', Input::get('searchText'));
            $counter = 0;
