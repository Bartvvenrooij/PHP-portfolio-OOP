...
?>
    ...
    <section class="comment">
        <?php
        if ($search == false) {
            if (count($comment->getCommentsByType('Css')) > 0) {
                foreach ($comment->getCommentsByType('Css') as $com) {
                    echo '<article>';
                    ...
                    if (Session::exists('user')) {
                        if ($user->getGroup() == 'Administrator') {
                            echo '<a href="?link=8&page=2&deleteComment=' . $com->idcomments . '">Delete comment</a>';
                        }
                    }
                    echo '</p></article>';
                }
            ...
            $results = $comment->getCommentsByMessage('Css', Input::get('searchText'));
            $counter = 0;
