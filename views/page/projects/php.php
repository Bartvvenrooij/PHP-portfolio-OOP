...
?>
    ...
    <section class="comment">
        <?php
        if ($search == false) {
            if (count($comment->getCommentsByType('Php')) > 0) {
                foreach ($comment->getCommentsByType('Php') as $com) {
                    echo '<article>';
                    ...
                    if (Session::exists('user')) {
                        if ($user->getGroup() == 'Administrator') {
                            echo '<a href="?link=8&page=3&deleteComment=' . $com->idcomments . '">Delete comment</a>';
                        }
                    }
                    echo '</p></article>';
                }
            ...
            $results = $comment->getCommentsByMessage('Php', Input::get('searchText'));
            $counter = 0;
