...
?>
    ...
    <section class="comment">
        <?php
        if ($search == false) {
            if (count($comment->getCommentsByType('Java')) > 0) {
                foreach ($comment->getCommentsByType('Java') as $com) {
                    echo '<article>';
                    ...
                    if (Session::exists('user')) {
                        if ($user->getGroup() == 'Administrator') {
                            echo '<a href="?link=8&page=4&deleteComment=' . $com->idcomments . '">Delete comment</a>';
                        }
                    }
                    echo '</p></article>';
                }
            ...
            $results = $comment->getCommentsByMessage('Java', Input::get('searchText'));
            $counter = 0;
